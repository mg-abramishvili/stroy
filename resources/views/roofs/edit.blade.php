...
@section('content')
    ...
    <div class="px-4 py-4">
        ...
        <form action="/roofs/{{$roof->id}}" method="post" enctype="multipart/form-data">@csrf
            @method('PUT')
            ...
            <div class="row align-items-center mb-2">    
                ...
                <dd class="col-sm-9">
                    <input type="text" class="form-control" name="name" value="{{$roof->name}}">
                    @if ($errors->has('name'))
            ...
            <div class="row align-items-center mb-2">    
                ...
                <dd class="col-sm-9">
                    <input type="text" class="form-control" name="score" value="{{$roof->score}}">
                    @if ($errors->has('score'))
                        <div class="alert alert-danger">
                            Укажите счёт
                        </div>
        ...
        $('.gallery').filepond({
            ...
            server: {
                ...
                process: (fieldName, file, metadata, load, error, progress, abort, transfer, options) => {
                    ...
                    request.open('POST', '/roofs/file/upload');
            ...
            files: [
            @if(isset($roof->gallery))
              @foreach($roof->gallery as $image)
                {
                    source: '{{ $image }}',
                    options: {
                        type: 'local',
